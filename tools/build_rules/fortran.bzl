def _single_fortran_object_impl(ctx):
    ...
    # load all toolchain flags

    cmd = toolchain_cflags + ['-c', ctx.file.src.path, '-o', ctx.outputs.pic_o.path]
    filtered_cmd = []
    # Strip out C/C++ specific flags from our toolchain
...
def _define_fortran_output(attrs):
    if not attrs.src.name.endswith('.f'):
        fail('Fortran files must in \'.f\'', 'src')
...
def fortran_library(name, srcs, deps = [], visibility = None):
    """ Construct a shared library from a fortran project.

    Args:
        srcs: List of fortran files all of which must end in .f
        deps: any dependencies in terms of bazel (cc_library, etc)
    """
    pic_o_files = []
    for src in srcs:
